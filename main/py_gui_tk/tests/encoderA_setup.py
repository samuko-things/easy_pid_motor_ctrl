...
class SetPwmCardFrame(SetCardFrame):
    def __init__(self, parentFrame):
      super().__init__(parentFrame, "PWM_A", ctrlPwm)
...
class MotorPosCanvas(customtkinter.CTkCanvas):
    ...
    def draw_motor_ang_pos(self):
      global motorIsOn, motorOnDuration, motorOnStartTime, initialTheta, theta, angPosA, angVelA
      ...
      theta = round(self.absAngDeg(angPosA),2)
...
class App(customtkinter.CTk):
    ...
    def sendPwmCtrl(self):
        ...
        if motorIsOn:
          ...
          isSuccess = serClient.send("pwm", ctrlPwm, 0)
          if isSuccess:
            motorIsOn = True
